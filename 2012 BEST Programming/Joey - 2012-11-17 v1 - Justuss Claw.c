...
void moveRobot(int drive, int arm, int turret, int claw, int clawTwist, int wrist) {
	if(SensorValue(driveS)) {
		if(motor[drive] > 0 && !driveSl) {
			driveS++;
			driveSl =  true;
		}
		if(motor[drive] <= 0 && !driveSl) {
...
task manipulation() {
	while(true) {
		...
		if(vexRT[Btn5U]) { // Retractable Arm
			motor[stretch] = 127;
			} else if(vexRT[Btn5D]) {
			motor[stretch] = -127;
			} else{
